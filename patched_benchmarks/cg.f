...
c---------------------------------------------------------------------
      ...
      double precision   t, t1, mflops, tmax
      character          class
      logical            verified
...
c     >          ' - CG Benchmark', /)
c 1001 format(' Size: ', i11 )
c 1002 format(' Iterations: ', i5 )
...
c 2000 format(' Initialization time = ',f15.3,' seconds')
...
c     aelt(naz)
      ...
      if (i_n .ne. 0) then
        ...
        call launch_fi_thread(a,memsize,i_n,exec_t,t_rand,s_rand)
      endif
...
c         err = abs( zeta - zeta_verify_value)
 ...
 401     format('NO VERIFICATION PERFORMED')
     ...
     &                 + 3. ) / t / 1000000.0
      ...
      write(*,40) 'Time in seconds            ', t
40    format(A15,' =',F9.4)

      write(*,41) 'kernel seconds            ', timer_read(T_init)
41    format(A15,' =',F9.4)


c         call print_results('CG', class, na, 0, 0,
c     >                      niter, t,
