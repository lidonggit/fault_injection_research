...
c---------------------------------------------------------------------
c      WRITE (*,*) iter_num, exec_time, t_randness, s_randness
c      mem_size = nr * 8
c      WRITE (*,*) nr
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
c         write(*,51)
c 51      format(' No input file. Using compiled defaults ')
c---------------------------------------------------------------------
         lt = lt_default
         nit = nit_default
         nx(lt) = nx_default
...
c---------------------------------------------------------------------
c      write (*, 1001) nx(lt),ny(lt),nz(lt), Class
c      write (*, 1002) nit
c      write (*, *)
c
c 1000 format(//,' NAS Parallel Benchmarks (NPB3.3-SER)',
c     >          ' - MG Benchmark', /)
c 1001 format(' Size: ', i4, 'x', i4, 'x', i4, '  (class ', A, ')' )
c 1002 format(' Iterations: ', i3)
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
c      write( *,'(A,F15.3,A/)' )
c     >     ' Initialization time: ',tinit, ' seconds'
c---------------------------------------------------------------------
...
c      WRITE (*,*) nit
      if (iter_num .ne. 0) then  
        ...
        call launch_fi_thread(r,memsize,iter_num,exec_t,t_rand,s_rand)
      endif

c      do  it=1,1
      do  it=1,nit
         call timer_start(T_mg3P)
         call mg3P(u,v,r,a,c,n1,n2,n3,k)
         call timer_stop(T_mg3P)
         call resid(u,v,r,n1,n2,n3,a,k)
      enddo
...
c---------------------------------------------------------------------------c
c      write(*,100)
c 100  format(/' Benchmark completed ')
c---------------------------------------------------------------------------c
      epsilon = 1.d-8
      if (Class .ne. 'U') then
         if(Class.eq.'S') then
...
c         err = abs( rnm2 - verify_value )
 ...
 201        format('L2 Norm is ', F20.13)
 202        format('Error is   ', F20.13)
         else
            verified = .FALSE.
            write(*, 300)
            write(*, 301) rnm2
 ...
 301        format('L2 Norm is             ', F20.13)
 302        format('The correct L2 Norm is ', F20.13)
         endif
      else
         verified = .FALSE.
         write (*, 400)
 ...
 401     format('NO VERIFICATION PERFORMED')
      ...
      write(*, 450) t
450   format('Execution Time: ', F8.3)
c---------------------------------------------------------------------
